...
using lawyer = std::vector<weights>;
using neural_network = std::vector<lawyer>;
...
std::ofstream f;
void save_data(std::vector<double> err, std::string fileName){
    f.open(fileName);
    if (f.eof())
    {
...
float random(float a, float b)				// returns a random float number between a and b
{
...
neural_network create_neural_network(int n_inputs, int n_lawyers, std::vector<int> & n_neurons)
{
	if (n_lawyers != n_neurons.size())	std::cerr << "Bad neural network parameters!" << std::endl;
	// -- Neural network basic structure --
	neural_network network(n_lawyers);
	
	// -- Neural network hidden lawyers and output lawyer initialization -- 
	for (int k = 0; k < n_lawyers; k++)
	{
		network[k] = lawyer(n_neurons[k]);		//! allocate space for neurons
...
features delta_hidden(features &output, lawyer &w_lawyer, features &deltas)
{
	features delta = output;
	for (int j = 0; j < output.size(); j++)	// neuron by neuron in current lawyer
	{
		float error = 0.0;
		for (int n = 0; n < w_lawyer.size(); n++)	// neuron by neuron in next lawyer
			error += w_lawyer[n][j] * deltas[n];
...
void train_network(neural_network &network, training &examples, expected &r_outputs, float l_rate, int n_iterations)	// + n_outputs
{
	...
	save_data(errors, "error_data.csv");	// save to file
	//return results;
}
...
*/
	...
	std::vector<int> n_neurons = {8,n_outputs};
	neural_network nn = create_neural_network(n_inputs, n_neurons.size(), n_neurons);
	print_network(nn);
